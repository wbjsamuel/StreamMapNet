...
from .distance import pairwise, chamfer_distance, frechet_distance, iou_distance, pairwise_chamfer_distance
from ..io import io
...
def _inject(num_gt, pred, tp, idx_match_gt, confidence, distance_threshold, object_type):
    ...
    confidence_thresholds = confidence[sorted_idx][np.asarray([taken_idx[t] for t in taken])]
...
def evaluate(ground_truth, predictions, verbose=True, only_lc=False):
    r"""
    ...
    for token in tqdm(gts.keys(), desc='calculating distances:', ncols=80, disable=not verbose):
        gt_pts = [gt['points'] for gt in gts[token]['lane_centerline']]
        pred_pts = [pred['points'] for pred in preds[token]['lane_centerline']]
        if gt_pts == [] :
            distance_matrixs['frechet'][token]=np.ones((len(gt_pts), len(pred_pts)), dtype=np.float64) * 1024  
            distance_matrixs['chamfer'][token]=np.ones((len(gt_pts), len(pred_pts)), dtype=np.float64) * 1024            
        else:            
            dist_chamfer, dist_chamfer_relaxed = pairwise_chamfer_distance(gt_pts, pred_pts, relax=True)
            distance_matrixs['chamfer'][token] = dist_chamfer

            mask = dist_chamfer_relaxed < THRESHOLDS_FRECHET[-1]
    ...
    metrics['OpenLane-V2 Score']['DET_l'] = mAP_LCs.mean()

    mAP_chamfer_LCs = _mAP_over_threshold(
        gts=gts, 
        preds=preds, 
    ...
    metrics['OpenLane-V2 Score']['DET_l_chamfer'] = mAP_chamfer_LCs.mean()

    if only_lc:
        return metrics['OpenLane-V2 Score']
    ...
    mAP_LCLCs = _mAP_topology_lclc(gts, preds, THRESHOLDS_FRECHET)
    mAP_LCTEs = _mAP_topology_lcte(
        gts,
        preds,
        {'lane_centerline': THRESHOLDS_FRECHET, 'traffic_element': THRESHOLDS_IOU},
    )

    metrics['OpenLane-V2 Score']['TOP_ll'] = np.mean(mAP_LCLCs)

    metrics['OpenLane-V2 Score']['TOP_lt'] = np.mean(mAP_LCTEs)
...
def evaluate_map_element(ground_truth, predictions, verbose=True):
    if isinstance(ground_truth, str):
        ground_truth = io.pickle_load(ground_truth)
    ...
    for token in ground_truth.keys():
        ...
        if predictions is None:
            preds[token] = gts[token]
            for i, _ in enumerate(preds[token]['map_element']):
                preds[token]['map_element'][i]['confidence'] = np.float32(1)
        else:
            preds[token] = predictions[token]['predictions']
    ...
    for token in tqdm(gts.keys(), desc='calculating distances:', ncols=80, disable=not verbose):
        gt_pts = [gt['points'] for gt in gts[token]['map_element']]
        pred_pts = [pred['points'] for pred in preds[token]['map_element']]
    ...
    mAP_LCs = [_mAP_over_threshold(
        ...
        distance_matrixs=distance_matrixs['chamfer'], 
        distance_thresholds=THRESHOLDS_CHAMFER,
        object_type='map_element',
        filter=lambda x: x['type'] == idx,
        inject=False, # save tp for eval on graph
    ).mean() for idx in [0,1,2]] 
    metrics['OpenLane-V2 Score']['mAP_divider'] = mAP_LCs[0]
    metrics['OpenLane-V2 Score']['mAP_ped_crossing'] = mAP_LCs[1]
    metrics['OpenLane-V2 Score']['mAP_boundary'] = mAP_LCs[2]
    metrics['OpenLane-V2 Score']['mAP'] = np.mean(mAP_LCs)

    return metrics

def evaluate_lanesegment(ground_truth, predictions, verbose=True):
    ...
    for token in tqdm(gts.keys(), desc='calculating distances:', ncols=80, disable=not verbose):
        distance_matrix_chamfer_centerline, distance_matrix_frechet_centerline = conduct_distance_matrixs(gts, preds, token, 'lane_centerline')
        # distance_matrix_chamfer_left, distance_matrix_frechet_left = conduct_distance_matrixs(gts, preds, token, 'left_divider')
        # distance_matrix_chamfer_right, distance_matrix_frechet_right = conduct_distance_matrixs(gts, preds, token, 'right_divider')
        # distance_matrixs['frechet'][token] = (distance_matrix_frechet_centerline + distance_matrix_frechet_left + distance_matrix_frechet_right)/3
        # distance_matrixs['chamfer'][token] = (distance_matrix_chamfer_centerline + distance_matrix_chamfer_left + distance_matrix_chamfer_right)/3
        distance_matrixs['frechet'][token] = distance_matrix_frechet_centerline
        distance_matrixs['chamfer'][token] = distance_matrix_chamfer_centerline
    ...
    metrics = evaluate_lanesegment_single(gts, preds, distance_matrixs, 'lane_centerline', metrics)

    return metrics

def evaluate_lanesegment_single(gts,preds,distance_matrixs,line_type,metrics):
    mAP_chamfer_LCs = _mAP_over_threshold(
        gts=gts, 
        preds=preds, 
        distance_matrixs=distance_matrixs['chamfer'], 
    ...
    mAP_frechet_LCs = _mAP_over_threshold(
        gts=gts, 
        preds=preds, 
        distance_matrixs=distance_matrixs['frechet'],
        distance_thresholds=THRESHOLDS_FRECHET,
    ...
    metrics['OpenLane-V2 Score'][f'{line_type}_chamfer'] = mAP_chamfer_LCs
    metrics['OpenLane-V2 Score'][f'{line_type}_frechet'] = mAP_frechet_LCs
    metrics['OpenLane-V2 Score'][f'TOP_ll'] = np.mean(mAP_LCLCs)
    return metrics

def conduct_distance_matrixs(gts, preds, token, line_type):
    gt_pts = [gt['points'] for gt in gts[token][line_type]]
    pred_pts = [pred['points'] for pred in preds[token][line_type]]
    if gt_pts == []:
        distance_matrix_chamfer = np.ones((len(gt_pts), len(pred_pts)), dtype=np.float64) * 1024
        distance_matrix_frechet = np.ones((len(gt_pts), len(pred_pts)), dtype=np.float64) * 1024
    else:            
        dist_chamfer, dist_chamfer_relaxed = pairwise_chamfer_distance(gt_pts, pred_pts, relax=True)
        distance_matrix_chamfer = dist_chamfer

        mask = dist_chamfer_relaxed < THRESHOLDS_FRECHET[-1]
        distance_matrix_frechet = pairwise(
            gt_pts,
            pred_pts,
            frechet_distance,
            mask=mask,
    ...
    return distance_matrix_chamfer, distance_matrix_frechet
