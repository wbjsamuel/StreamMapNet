...
def pairwise_chamfer_distance(gts, preds, relax=False):
    src = torch.tensor(gts, dtype=torch.float32, device='cpu')
    dst = torch.tensor(preds, dtype=torch.float32, device='cpu')
    ...
    src2dst_distance = torch.min(distance, dim=3)[0].mean(-1)
    dst2src_distance = torch.min(distance, dim=2)[0].mean(-1)
    distance = (src2dst_distance + dst2src_distance) / 2

    if relax:
        ego_distances = torch.norm(src, dim=2).min(-1)[0]
        relaxation_factors = 1 - 5e-3 * ego_distances
        relaxation_factors = torch.clamp(relaxation_factors, 0.5, 1.0)
        relaxed_dist = distance * relaxation_factors.unsqueeze(1)
        return distance.numpy(), relaxed_dist.numpy()
