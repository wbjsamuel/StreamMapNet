...
def calculate_type_error(array1, array2):
    if len(array1) != len(array2):
        raise ValueError("Arrays must have the same length.")
    length = len(array1)
    count = sum(1 for i in range(length) if array1[i] == array2[i])
    similarity = 1 - (count / length)
    return similarity


def calculate_distance_error(array1, array2):
